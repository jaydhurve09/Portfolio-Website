    ...
    id='about' className='w-full p-[12%] py-10 scroll-mt-20'>
      ...
      className='flex w-full flex-col lg:flex-row items-center gap-20 my-20'>
        ...
        className='w-64 sm:w-80 rounded-3xl max-w-none'>
            <Image src={assets.jay_image} alt='' className='w-full rounded-3xl' />
        </motion.div>
        <motion.div
        initial={{opacity: 0}}
        whileInView={{opacity: 1}}
        ...
        className='flex-1'>
            ...
            className='grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-2xl '>
                {
                    infoList.map(({icon, iconDark, title, description}, index)=>(
                        ...
                        className='border-[0.5px] border-gray-400 rounded-xl p-6 cursor-pointer hover:bg-lightHover hover:-translate-y-1 duration-500 hover:shadow-black dark:border-white dark:hover:shadow-lightHover dark:hover:bg-darkHover' key={index}>
            ...
            className='flex items-center gap-3 sm:gap-5'>
              {toolsData.map((tool, index)=>(
                ...
                className='flex items-center justify-center w-12 sm:w-14 aspect-square border border-gray-400 rounded-lg cursor-pointer hover:-translate-y-1 duration-500' key={index}><Image src={tool} className='w-5 sm:w-7'/></motion.li>
              ))}
            </motion.ul>
        </motion.div>
      </motion.div>
