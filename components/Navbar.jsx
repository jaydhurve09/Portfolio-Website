...
const Navbar = ({isDarkMode, setIsDarkMode}) => {
  ...
  return (
    ...
    <div className='fixed top-0 right-0 w-11/12 -z-10 translate-y-[-80%]'>
        <Image src={assets.header_bg_color} alt='' className='w-full dark:hidden'/>
    </div>
    <nav className={`w-full fixed px-5 lg:px-8 xl:px-[8%] py-4 flex items-center justify-between z-50 ${isScroll?"bg-white bg-opacity-50 backdrop-blur-lg dark:bg-darkTheme dark:shadow-white/20 shadow-sm" : ""}`}>
        <a href="#home">
            <Image src={isDarkMode ? assets.jay_logo_dark : assets.jay_logo} alt='jay-logo' className='w-28 cursor-pointer mr-14'/>
        </a>
        ...
        <ul ref={sideMenuRef} className='flex md:hidden flex-col gap-4 px-10 py-20 fixed -right-64 top-0 bottom-0 w-64 z-50 h-screen bg-rose-50 transition duration-500 dark:bg-darkHover'>
            <div className='absolute right-6 top-6' onClick={closeMenu}>
                <Image src={isDarkMode ? assets.close_white : assets.close_black} alt='' className='w-5 cursor-pointer'/>
            </div>

            <li><a onClick={closeMenu} href="#home" className='font-Ovo'>Home</a></li>
